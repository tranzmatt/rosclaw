"""
RosBot — Natural language TurtleBot4 control via Claude API + rosbridge.

Usage:
    ROSBRIDGE_URL=ws://192.168.x.x:9090 python server.py
...
Environment variables:
    ...
    TWIST_STAMPED      Set true for TurtleBot3 Gazebo sim (default: false)
...
try:
    ...
    print("faster-whisper not installed — voice disabled. pip install faster-whisper")
...
def _make_twist_msg(linear_x: float, angular_z: float) -> tuple[str, dict]:
    """Return (msg_type, msg) for the correct Twist variant."""
    body = {
...
async def execute_tool(tool_name: str, tool_input: dict) -> str:
    try:
        if tool_name == "drive":
            ...
            return await ros_stop()
        elif tool_name == "get_position":
            return await ros_get_position()
        elif tool_name == "get_battery":
            return await ros_get_battery()
        elif tool_name == "list_topics":
            return await ros_list_topics()
        else:
...
- Ask for clarification if a command is ambiguous or dangerous
...
Keep replies short and conversational. Confirm what you did or why you couldn't."""
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    await websocket.send_json({
        "type": "status",
        "text": f"RosBot ready | {ROSBRIDGE_URL} | Twist{'Stamped' if TWIST_STAMPED else ''} | Voice: {'on' if whisper_model else 'off'}",
    })

    if not ros.connected:
        ok = await ros.connect()
        if ok:
            await websocket.send_json({"type": "status", "text": "✓ Connected to TurtleBot4"})
        else:
            await websocket.send_json({"type": "status", "text": "⚠ Cannot reach TurtleBot4 — check rosbridge URL"})
    ...
    try:
        while True:
            ...
            try:
                ...
                await websocket.send_json({"type": "error", "text": "Invalid ANTHROPIC_API_KEY."})
            except Exception as e:
